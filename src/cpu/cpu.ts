...
class CPU {
  ...
  read16(r1h: CPURegisterType, r1l: CPURegisterType): number  {
    return (this.register[r1h] << 8) | this.register[r1l]
  }
  ...
  write16(value: number, r1h: CPURegisterType, r1l: CPURegisterType): void {
    this.register[r1h] = (value >> 8)
    this.register[r1l] = (value & 0xFF)
  }
  ...
  //   const frameClock: number = (this.clock.M / 4) + GPUSettings.CYCLES_PER_FRAME
  //   let cycles: number = 0
  //   let operationCycles: number = 0
  //   let nextIs16bit: boolean = false

  //   do {
  //     //console.log('Current Clock:', this.clock.M, 'Frame Clock:', frameClock)
  //     this.clock.M = 0
  //     //if (ts < lastFrameTimeMS + (1000 / CLOCK_SPEED)) {
  //       //return requestAnimationFrame(c)
  //     //}

  //     const triggeredInterrupts = this.mmu.IE & this.mmu.IF
  //     if (this.IME && triggeredInterrupts) {
  //         const interruptVector = triggeredInterrupts & -triggeredInterrupts // Get lowest priority interrupt
  //         this.mmu.IF &= ~interruptVector // Acknowledge it
  //         this.opcodes.RST(interruptVector)
  //     }
  ...
  //     // console.log('PC:', this.register.PC.toString(16).toUpperCase())

  //       // Check if it's an 8-bit or 16-bit (0xCBxx) instruction
  //     if (nextIs16bit) this.execute16bit(opcode)
  //     else this.execute8bit(opcode)
...
//   public cycle(): void {
//     let frameStartCycles = (this.clock.M / 4)  // Track start of frame

//     while ((this.clock.M / 4) - frameStartCycles < GPUSettings.CYCLES_PER_FRAME) {
//         // Handle interrupts if IME is enabled
//         if (this.IME && this.mmu.IE && this.mmu.IF) {
//             this.handleInterrupts()
//         }

//         this.register.PC &= CPUSettings.MAX_FRAME_CYCLES

//         // Fetch next opcode at PC
//         //console.log(`PC: 0x${this.register.PC.toString(16).toUpperCase()}`)
//         const opcode = this.mmu.read(this.register.PC)
        
//         // Check if it's an 8-bit or 16-bit (0xCBxx) instruction
//         opcode == 0xCB ? this.execute16bit(opcode) :this.execute8bit(opcode)
//         console.log('Instruction:', opcode.toString(16).toUpperCase(), opcode == 0xCB ? '16 bit' : '8 bit')
//         console.log(`A: ${this.register.A.toString(16).toUpperCase()}, F: ${this.register.F.toString(16).toUpperCase()}, B: ${this.register.B.toString(16).toUpperCase()}, C: ${this.register.C.toString(16).toUpperCase()}, D: ${this.register.D.toString(16).toUpperCase()}, E: ${this.register.E.toString(16).toUpperCase()}, H: ${this.register.H.toString(16).toUpperCase()}, L: ${this.register.L.toString(16).toUpperCase()}, SP: ${this.register.SP.toString(16).toUpperCase()}, PC: ${this.register.PC.toString(16).toUpperCase()}, (${this.mmu.read(this.register.PC).toString(16).toUpperCase()} ${this.mmu.read(this.register.PC+1).toString(16).toUpperCase()} ${this.mmu.read(this.register.PC+2).toString(16).toUpperCase()} ${this.mmu.read(this.register.PC+3).toString(16).toUpperCase()})`)
//         console.log('register:', this.register)
//         // GPU update (executed per instruction, but only draws when needed)
//         this.gpu.tick()

//         // Handle timers (div register, timer overflow, etc.)
//         this.handleTimer();
//     }

//     // Once per frame, we trigger a VBlank (start of new screen draw)
//     this.mmu.IF |= 0x01;  // VBlank interrupt request
//     //this.debug.readSerialOutput()
// }
  ...
  public tick() {
    ...
    // Check if it's an 8-bit or 16-bit (0xCBxx) instruction
    if (opcode == 0xCB) this.execute16bit(opcode) 
    else this.execute8bit(opcode)

    console.log(`A: ${formatHex(this.register.A.toString(16).toUpperCase())} F: ${formatHex(this.register.F.toString(16).toUpperCase())} B: ${formatHex(this.register.B.toString(16).toUpperCase())} C: ${formatHex(this.register.C.toString(16).toUpperCase())} D: ${formatHex(this.register.D.toString(16).toUpperCase())} E: ${formatHex(this.register.E.toString(16).toUpperCase())} H: ${formatHex(this.register.H.toString(16).toUpperCase())} L: ${formatHex(this.register.L.toString(16).toUpperCase())} SP: ${formatHex(this.register.SP.toString(16).toUpperCase())} PC: 00:0${this.register.PC.toString(16).toUpperCase()} (${formatHex(this.mmu.read(this.register.PC).toString(16).toUpperCase())} ${formatHex(this.mmu.read(this.register.PC+1).toString(16).toUpperCase())} ${formatHex(this.mmu.read(this.register.PC+2).toString(16).toUpperCase())} ${formatHex(this.mmu.read(this.register.PC+3).toString(16).toUpperCase())}) (Instr: ${opcode.toString(16).toUpperCase()} ${opcode == 0xCB ? '16 bit' : '8 bit'})`)
    // console.log('register:', this.register)
    // GPU update (executed per instruction, but only draws when needed)
    this.gpu.tick()
  ...
  public cycle() {
    ...
    const frame = () => {
      ...
      let elapsedMs = now - lastFrameTime
      lastFrameTime = now

      let cyclesToRun = (elapsedMs / 1000) * CPUSettings.CLOCK_SPEED
      let cyclesThisFrame = 0

      while (cyclesThisFrame < CPUSettings.MAX_FRAME_CYCLES && cyclesThisFrame < cyclesToRun) {
       this.tick()
       cyclesThisFrame += this.clock.M;
      }
      ...
      requestAnimationFrame(frame); // Continue running
    }

    frame()
  }

  private execute8bit(opcode: number) {
    //console.log('8-bit instruction: ', opcode)
    //console.log(`Opcode (8-bit): ${opcode.toString(16)} | Cycles: ${this.clock.M}`);
   ...
   // We interpret all opcodes as 16-bit by masking with FFFF; 
  // then we distinguish between 8-bit and 16-bit with an additional nested switch case 
  private execute16bit(opcode: number) {
    //console.log('16-bit instruction: ', opcode)
    //console.log(`Opcode (16-bit): ${opcode.toString(16)} | Cycles: ${this.clock.M}`);
